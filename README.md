...
You own **vcs**, a small source-control service in this repo. Its code is correct
but has no safeguards, so under load it crashes or degrades — and telemetry is
the only way to see when and why.
...
dgs is a genuine black box: its source is **not in this repo**. The stack pulls
it as a prebuilt image, so you cannot read the oracle to shortcut the answer —
the telemetry you build is the only way through.
